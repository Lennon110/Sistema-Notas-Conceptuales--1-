...
/** Singleton: única fuente de verdad en memoria para toda la aplicación. */
export class SistemaGestion {
  ...
  public buscarPorCodigo(codigo: string): NotaConceptual | null {
    return this._notasConceptuales.find((n) => n.codigo === codigo) ?? null;
  }
  ...
  public obtenerEstadisticas(convocatoriaId: string): IEstadisticasDashboard {
    const notas = this._notasConceptuales.filter((n) => n.convocatoriaId === convocatoriaId);
    const notasAprobadas = notas.filter((n) => n.estado === EstadoNota.APROBADA);
    return {
      numeroNotas: notas.length,
      presupuestoTotal: notasAprobadas.reduce((acc, n) => acc + n.calcularPresupuestoTotal(), 0),
      notasAprobadas: notasAprobadas.length,
